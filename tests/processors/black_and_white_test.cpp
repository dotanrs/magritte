...
void test_black_and_white() {
    ...
    const FileData grayscale =
        black_and_white_processor().apply(image, {"1"});
    expect(
    ...
    const FileData brightened =
        black_and_white_processor().apply(image, {"1.2"});
    expect(
    ...
    const FileData clamped =
        black_and_white_processor().apply(white, {"2"});
    expect(
    ...
    const FileData black =
        black_and_white_processor().apply(image, {"0"});
    expect(
    ...
    const auto arguments =
        black_and_white_processor().parse_arguments("black-and-white 1.25");
    expect(
        arguments ==
    ...
    expect(
        !black_and_white_processor().parse_arguments("contrast 2").has_value(),
        "black-and-white should decline another processor's command"
    );

    bool rejected_negative_multiplier = false;
    try {
        black_and_white_processor().validate({"-0.5"});
