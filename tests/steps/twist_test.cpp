...
void test_twist() {
    ...
    const FileData twisted = twist_step().apply(
        image,
    ...
    const FileData reversed = twist_step().apply(
        image,
        {"50", "50", std::to_string(-std::acos(-1.0))}
    );
    expect(
    ...
    const FileData identity = twist_step().apply(
        image,
        {"25", "75", "0"}
    );
    expect(
    ...
    const FileData localized = twist_step().apply(
        localized_source,
        {"50", "50", "4", "20"}
    );
    expect(
...
void test_twist_debug_hints() {
    ...
    const FileData processed = twist_step().apply(image, arguments);
    const FileData debugged = twist_step().add_debug_hints(
        twist_step().apply(std::move(image), arguments),
        arguments
    );
