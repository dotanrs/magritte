...
const FileData enlarged = fisheye_step().apply(
    image(),
    {"50", "50", "1", "40"}
);
expect(
...
const FileData shrunk = fisheye_step().apply(
    image(),
    {"50", "50", "-0.5", "40"}
);
expect(
...
const FileData offset = fisheye_step().apply(
    image(),
    {"25", "50", "1", "60"}
);
expect(
...
const FileData identity = fisheye_step().apply(
    image(),
    {"50", "50", "0"}
);
expect(
...
const FileData default_radius = fisheye_step().apply(
    image(),
    {"50", "50", "1"}
);
const FileData explicit_default_radius = fisheye_step().apply(
    image(),
    {"50", "50", "1", "100"}
);
expect(
...
const FileData processed = fisheye_step().apply(image, arguments);
const FileData debugged = fisheye_step().add_debug_hints(
    fisheye_step().apply(std::move(image), arguments),
    arguments
);
...
const FileData other_step = rotate_step().add_debug_hints(
    blank_image(2, 2),
    {"1"}
);
expect(
    red_values(other_step) ==
    std::vector<std::uint8_t>{0, 0, 0, 0},
    "steps without debug hints should leave the image unchanged"
);
}
