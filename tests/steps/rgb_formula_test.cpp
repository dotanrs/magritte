...
const FileData processed = rgb_formula_step().apply(
    std::move(image),
    {"r", "r * 2 - g"}
);
...
const FileData horizontal = rgb_formula_step().apply(
    blank_image(3, 1),
    {"r", "(U + 1) * 100"}
);
expect(
...
const FileData vertical = rgb_formula_step().apply(
    blank_image(1, 3),
    {"r", "(V + 1) * 100"}
);
expect(
...
const FileData single_pixel = rgb_formula_step().apply(
    blank_image(1, 1),
    {"r", "100 + U + V"}
);
expect(
...
const FileData distance = rgb_formula_step().apply(
    blank_image(3, 3),
    {"r", "D * 100"}
);
expect(
...
const FileData angle = rgb_formula_step().apply(
    blank_image(3, 3),
    {"r", "round((A + PI) * 10)"}
);
expect(
...
const FileData offset_polar = rgb_formula_step().apply(
    blank_image(3, 3),
...
const FileData percentage_origin = rgb_formula_step().apply(
    blank_image(5, 5),
    {"r", "D * 10", "25", "75"}
);
expect(
...
const FileData localized = rgb_formula_step().apply(
...
const FileData trigonometry = rgb_formula_step().apply(
    blank_image(1, 1),
...
const FileData arithmetic = rgb_formula_step().apply(
    blank_image(1, 1),
    {
        "r",
...
const FileData clamped = rgb_formula_step().apply(
    blank_image(1, 1),
    {"r", "clamp(300, 200, 10)"}
);
expect(
...
expect(
    ...
    "step validation should allow explicitly prefixed macro variables"
);

try {
    static_cast<void>(rgb_formula_step().apply(
        blank_image(1, 1),
        {"r", "macro_missing"}
    ));
...
const FileData coordinate_processed = rgb_formula_step().apply(
    blank_image(2, 2),
    {
        "("
        "127 + 127 * sin(PI * U), "
...
const FileData subset = rgb_formula_step().apply(
    FileData{
        .width = 1,
        .height = 1,
        .pixels = {
