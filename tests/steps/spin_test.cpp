...
void test_spin() {
    ...
    const FileData spun = spin_step().apply(
        image,
        {"50", "50", "90"}
    );
    ...
    const FileData identity = spin_step().apply(
        image,
        {"25", "75", "0", "20"}
    );
    expect(
    ...
    const FileData localized = spin_step().apply(
        localized_source,
        {"50", "50", "90", "20"}
    );
    expect(
...
void test_spin_debug_hints() {
    ...
    const FileData debugged = spin_step().add_debug_hints(
        spin_step().apply(std::move(image), arguments),
        arguments
    );
