// Step: `flow-lines <spacing> <steps> <step-size> <width> <#RRGGBB>
// [opacity] = (<VX>, <VY>)`.
...
namespace {
    ...
    std::optional<std::vector<std::string>> parse_command(
        ...
        const std::string_view value =
            step_argument_parse::trim(command);
        ...
        auto words = step_argument_parse::split_words(
            step_argument_parse::trim(value.substr(0, equals))
        );
        ...
        words.emplace_back(
            step_argument_parse::trim(value.substr(equals + 1))
        );
        return words;
    }
    ...
    class FlowLinesStep final : public ImageStep {
    public:
...
const ImageStep &flow_lines_step() {
    static const FlowLinesStep step;
    return step;
}
