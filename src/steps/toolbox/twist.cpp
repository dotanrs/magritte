...
namespace {
    ...
    TwistArguments parse_twist_arguments(
        ...
        if (result.radius_percent && *result.radius_percent <= 0.0) {
            throw std::invalid_argument(
                "twist radius must be greater than 0"
            );
        }
        return result;
    }
    ...
    double radius_falloff(double distance, double radius) {
    ...
    FileData add_twist_debug_hints(
        FileData data,
        ...
        for (double distance = 0.0;
             ...
             distance += 0.5) {
            const double falloff = arguments.radius_percent
                ? radius_falloff(distance, guide_radius)
                : 1.0;
            const double angle =
                distance * arguments.force / 100.0 * falloff;
            mark_disk(
                data,
    ...
    FileData apply_twist(
        ...
        if (arguments.radius_percent && !std::isfinite(radius)) {
            throw std::invalid_argument("twist radius is too large");
        }
        FileData result{
            .width = data.width,
            .height = data.height,
        ...
        for (std::size_t y = 0; y < data.height; ++y) {
            for (std::size_t x = 0; x < data.width; ++x) {
                ...
                double falloff = 1.0;
                if (arguments.radius_percent) {
                    falloff = radius_falloff(distance, radius);
                }
    ...
    class TwistStep final : public ImageStep {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments =
                step_argument_parse::after_keyword(command, "twist");
            if (arguments) {
                validate(*arguments);
            }
            return arguments;
...
const ImageStep &twist_step() {
    static const TwistStep step;
    return step;
}
