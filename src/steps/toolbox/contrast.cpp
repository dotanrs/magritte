...
namespace {
    ...
    class ContrastStep final : public ImageStep {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments =
                step_argument_parse::after_keyword(command, "contrast");
            if (arguments) {
                validate(*arguments);
            }
            return arguments;
...
const ImageStep &contrast_step() {
    static const ContrastStep step;
    return step;
}
