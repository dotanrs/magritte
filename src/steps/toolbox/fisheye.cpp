// Step: `fisheye <x> <y> <amount> [radius]`.
...
namespace {
    ...
    class FisheyeStep final : public ImageStep {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments =
                    step_argument_parse::after_keyword(command, "fisheye");
            if (arguments) {
                validate(*arguments);
            }
            return arguments;
...
const ImageStep &fisheye_step() {
    static const FisheyeStep step;
    return step;
}
