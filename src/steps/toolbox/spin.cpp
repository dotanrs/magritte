...
namespace {
    ...
    class SpinStep final : public ImageStep {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments =
                step_argument_parse::after_keyword(command, "spin");
            if (arguments) {
                validate(*arguments);
            }
            return arguments;
...
const ImageStep &spin_step() {
    static const SpinStep step;
    return step;
}
