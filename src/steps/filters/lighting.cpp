// Step: `lighting <preset> [strength]` or
...
namespace {
    ...
    class LightingStep final : public ImageStep {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments =
                step_argument_parse::after_keyword(command, "lighting");
            if (arguments) {
                validate(*arguments);
            }
            return arguments;
...
const ImageStep &lighting_step() {
    static const LightingStep step;
    return step;
}
