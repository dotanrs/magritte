// Processor: `black-and-white <brightness>`.
...
namespace {
    ...
    class BlackAndWhiteProcessor final : public ImageProcessor {
    public:
        ...
        [[nodiscard]] std::optional<std::vector<std::string>> parse_arguments(
            ...
            auto arguments = processor_argument_parse::after_keyword(
                command,
                "black-and-white"
            );
            if (arguments) {
...
const ImageProcessor &black_and_white_processor() {
    static const BlackAndWhiteProcessor processor;
    return processor;
}
