...
`magritte` is a C++20 command-line JPEG processing tool. Step commands are parsed
...
- `include/magritte/` contains the headers shared by the application and tests.
  Step declarations live in `include/magritte/steps/`, while
...
- `src/steps/` contains step implementations, grouped by behavior:
  ...
  - `loops/` repeatedly applies another step, feeding each result into the
    next iteration.
...
- `patterns/` contains runnable YAML patterns that demonstrate supported step
  commands.
- `CMakeLists.txt` declares both the CLI and step-test targets.

## Step types

Every step implements `ImageStep`, which separates four concerns:
recognizing a command, validating its arguments, applying it, and optionally
adding debug hints.

- Keyword steps use commands such as `blur 3`. They usually parse
  arguments with `step_argument_parse::after_keyword`.
- Assignment steps use commands such as `warp = (X, Y)`. When the
  left-hand side is a fixed keyword, derive from `AssignmentStep` to reuse
  recognition and validation flow. The RGB formula step is a special
  assignment step because its left-hand side may be any non-repeating
  subset of `r`, `g`, and `b`.
- Formula steps evaluate expressions independently for each output pixel.
...
- Localized steps add relative, bilinearly interpolated sampling of the
...
- Loop steps are wrappers around an existing step. `LoopStep`
  handles iteration and result feedback, while `LoopAssignmentStep`
  supplies the `loop-<name> <count> = ...` syntax. Prefer wrapping an existing
  step over duplicating its transformation.
- Drawing steps composite generated marks onto the input. They still obey
  the same parsing, validation, alpha-preservation, and image-invariant
  expectations as other steps.

The source folders describe behavior, not separate interfaces: all of these
types ultimately participate in the same `ImageStep` registry and
...
A new step is not complete without both its test and its `patterns/` example.
