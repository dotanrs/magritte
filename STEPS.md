...
Steps run sequentially from left to right. In a pattern file, put each
command in the ordered `steps` list. See the [usage guide](USAGE.md) for
...
Separate assignments behave differently because later steps see the
changes made by earlier steps.
